...
const main = () => {
  ...
  execSyncWithErrorHandling(`docker tag api3/airnode-feed:latest api3/airnode-feed:${version}`);
  execSyncWithErrorHandling(`docker tag api3/signed-api:latest api3/signed-api:${version}`);

  console.info('The images are built and ready to be published. Run the following commands to publish them:');
  console.info();
  console.info(`docker push api3/airnode-feed:${version} && docker push api3/airnode-feed:latest`);
  console.info(`docker push api3/signed-api:${version} && docker push api3/signed-api:latest`);
};
